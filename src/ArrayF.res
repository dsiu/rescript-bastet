...
module Make = (A: IMPL): ARRAY => {
  ...
  let zip: (array<'a>, array<'b>) => array<'c> = (xs, ys) => zip_with((a, b) => (a, b), xs, ys)
  ...
  (TRAVERSABLE with type t<'a> = array<'a> and type applicative_t<'a> = A.t<'a>)
  ...
  module Eq: EQ_F = (E: EQ) => {
    ...
    let eq = (xs, ys) =>
      A.length(xs) == A.length(ys) && A.every(((a, b)) => E.eq(a, b), zip(xs, ys))
  }

  module Ord: ORD_F = (O: ORD) => {
    include Eq(O)
    ...
    let compare = (xs, ys) =>
      ...
      | _ if A.length(xs) < A.length(ys) => #less_than
      | _ => #greater_than
      }
  }
  ...
  module Extend: EXTEND with type t<'a> = array<'a> = {
    ...
    let extend = (f, xs) => A.mapi((_, i) => f(A.slice(xs, ~start=i, ~end_=A.length(xs))), xs)
  }
