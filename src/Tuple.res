...
module type MAGMA_F = (First: MAGMA, Second: MAGMA) => (MAGMA with type t = (First.t, Second.t))

module type SEMIGROUP_F = (First: SEMIGROUP, Second: SEMIGROUP) =>
(SEMIGROUP with type t = (First.t, Second.t))

module type MONOID_F = (First: MONOID, Second: MONOID) => (MONOID with type t = (First.t, Second.t))

module type FUNCTOR_F = (T: TYPE) => (FUNCTOR with type t<'a> = (T.t, 'a))

module type APPLY_F = (S: SEMIGROUP) => (APPLY with type t<'a> = (S.t, 'a))

module type APPLICATIVE_F = (M: MONOID) => (APPLICATIVE with type t<'a> = (M.t, 'a))

module type MONAD_F = (M: MONOID) => (MONAD with type t<'a> = (M.t, 'a))

module type FOLDABLE_F = (T: TYPE) => (FOLDABLE with type t<'a> = (T.t, 'a))

module type EQ_F = (First: EQ, Second: EQ) => (EQ with type t = (First.t, Second.t))

module type SHOW_F = (First: SHOW, Second: SHOW) => (SHOW with type t = (First.t, Second.t))

module type TRAVERSABLE_F = (T: TYPE, A: APPLICATIVE) =>
(TRAVERSABLE with type t<'a> = (T.t, 'a) and type applicative_t<'a> = A.t<'a>)

module Magma: MAGMA_F = (First: MAGMA, Second: MAGMA) => {
  type t = (First.t, Second.t)

  let append = ((a, b), (a', b')) => (First.append(a, a'), Second.append(b, b'))
}

module Semigroup: SEMIGROUP_F = (First: SEMIGROUP, Second: SEMIGROUP) => {
  include Magma(First, Second)
}

module Monoid: MONOID_F = (First: MONOID, Second: MONOID) => {
...
(BITRAVERSABLE with type applicative_t<'a> = A.t<'a> and type t<'a, 'b> = ('a, 'b))
