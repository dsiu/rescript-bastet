...
module type SEMIGROUP_F = (S: SEMIGROUP) => (SEMIGROUP with type t = dual<S.t>)

module type MONOID_F = (M: MONOID) => (MONOID with type t = dual<M.t>)

module type MAGMA_ANY_F = (M: MAGMA_ANY) => (MAGMA_ANY with type t<'a> = dual<M.t<'a>>)

module type SEMIGROUP_ANY_F = (S: SEMIGROUP_ANY) => (SEMIGROUP_ANY with type t<'a> = dual<S.t<'a>>)

module type MONOID_ANY_F = (M: MONOID_ANY) => (MONOID_ANY with type t<'a> = dual<M.t<'a>>)

module type TRAVERSABLE_F = (A: APPLICATIVE) =>
(TRAVERSABLE with type t<'a> = dual<'a> and type applicative_t<'a> = A.t<'a>)
...
module Semigroup: SEMIGROUP_F = (S: SEMIGROUP) => {
  include Magma(S)
}

module Monoid: MONOID_F = (M: MONOID) => {
  include Semigroup(M)
...
module Semigroup_Any: SEMIGROUP_ANY_F = (S: SEMIGROUP_ANY) => {
  include Magma_Any(S)
}

module Monoid_Any: MONOID_ANY_F = (M: MONOID_ANY) => {
