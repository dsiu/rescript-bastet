...
module type MONOID = {
  @@ocaml.text(" A {!MONOID} only needs to be commutative with the {!empty} element. ")
...
module type MEDIAL_QUASIGROUP = {
  @@ocaml.text(" Every {!MEDIAL_QUASIGROUP} is isotopic to an {!ABELIAN_GROUP}. ")
...
module type LOOP = {
  @@ocaml.text(" A {!LOOP} only needs to be commutative with the {!empty} element. ")
...
module type LOOP_ANY = {
  @@ocaml.text(" A {!LOOP_ANY} only needs to be commutative with the {!empty} element. ")
...
module type GROUP = {
  @@ocaml.text(" Include {i only} either {!LOOP} or {!MONOID}. ")
...
module type GROUP_ANY = {
  @@ocaml.text(" Include {i only} either {!LOOP_ANY} or {!MONOID_ANY}. ")

  include LOOP_ANY

  include MONOID_ANY with type t<'a> := t<'a>

  let inverse: t<'a> => t<'a>
}

@ocaml.doc(" Every {!GROUP} is a {!LOOP}. ")
...
module Ordering = (O: ORD) => {
  ...
  and greater_than: (O.t, O.t) => bool = (a, b) => O.compare(a, b) == #greater_than

  and less_than_or_equal: (O.t, O.t) => bool = (a, b) => O.compare(a, b) != #greater_than

  and greater_than_or_equal: (O.t, O.t) => bool = (a, b) => O.compare(a, b) != #less_than
}
...
(BITRAVERSABLE with type applicative_t<'a> = A.t<'a>)
