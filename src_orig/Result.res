...
module type MAGMA_F = (T: TYPE, M: MAGMA) => (MAGMA with type t = result<M.t, T.t>)

module type MEDIAL_MAGMA_F = (T: TYPE, M: MAGMA) => (MEDIAL_MAGMA with type t = result<M.t, T.t>)

module type SEMIGROUP_F = (T: TYPE, S: SEMIGROUP) => (SEMIGROUP with type t = result<S.t, T.t>)

module type FUNCTOR_F = (T: TYPE) => (FUNCTOR with type t<'a> = result<'a, T.t>)

module type APPLY_F = (T: TYPE) => (APPLY with type t<'a> = result<'a, T.t>)

module type APPLICATIVE_F = (T: TYPE) => (APPLICATIVE with type t<'a> = result<'a, T.t>)

module type MONAD_F = (T: TYPE) => (MONAD with type t<'a> = result<'a, T.t>)

module type ALT_F = (T: TYPE) => (ALT with type t<'a> = result<'a, T.t>)

module type EXTEND_F = (T: TYPE) => (EXTEND with type t<'a> = result<'a, T.t>)

module type SHOW_F = (Ok: SHOW, Error: SHOW) => (SHOW with type t = result<Ok.t, Error.t>)

module type EQ_F = (Ok: EQ, Error: EQ) => (EQ with type t = result<Ok.t, Error.t>)

module type ORD_F = (Ok: ORD, Error: ORD) => (ORD with type t = result<Ok.t, Error.t>)

module type BOUNDED_F = (Ok: BOUNDED, Error: BOUNDED) =>
(BOUNDED with type t = result<Ok.t, Error.t>)

module type FOLDABLE_F = (T: TYPE) => (FOLDABLE with type t<'a> = result<'a, T.t>)

module type TRAVERSABLE_F = (T: TYPE, A: APPLICATIVE) =>
(TRAVERSABLE with type t<'a> = result<'a, T.t> and type applicative_t<'a> = A.t<'a>)

module type BITRAVERSABLE_F = (A: APPLICATIVE) =>
(BITRAVERSABLE with type t<'a, 'b> = result<'a, 'b> and type applicative_t<'a> = A.t<'a>)

module Magma: MAGMA_F = (T: TYPE, M: MAGMA) => {
  type t = result<M.t, T.t>
...
module Medial_Magma: MEDIAL_MAGMA_F = (T: TYPE, M: MAGMA) => {
  include Magma(T, M)
}

module Semigroup: SEMIGROUP_F = (T: TYPE, S: SEMIGROUP) => {
  include Magma(T, S)
}
...
module Bifunctor: BIFUNCTOR with type t<'a, 'b> = result<'a, 'b> = {
...
module Many_Valued_Logic = {
  @@ocaml.text(" Many valued logics in general have to relax certain constraints in order to
  ...
  module Bounded_Lattice = (Ok: BOUNDED_LATTICE, Error: BOUNDED_LATTICE) => {
    ...
    include (
      Bounded_Meet_Semilattice(Ok, Error): module type of Bounded_Meet_Semilattice(Ok, Error)
        with type t := t
    )
  }
  ...
  module Heyting_Algebra: HEYTING_ALGEBRA_F = (Ok: HEYTING_ALGEBRA, Error: HEYTING_ALGEBRA) => {
    ...
    include (
      Bounded_Distributive_Lattice(Ok, Error): module type of Bounded_Distributive_Lattice(
        Ok,
        Error,
      )
        with type t := t
    ...
    let not = a =>
      switch a {
      | Ok(a') if a' == Ok.top => Error(Error.bottom)
      | Ok(a') if a' == Ok.bottom => Error(Error.top)
      | Error(a') if a' == Error.top => Ok(Ok.bottom)
      | Error(a') if a' == Error.bottom => Ok(Ok.top)
      | a' => a'
      }

    let implies = (a, b) => join(!a, b)
  }
  ...
  module Boolean_Algebra = (Ok: BOOLEAN_ALGEBRA, Error: BOOLEAN_ALGEBRA) => {
    include Heyting_Algebra(Ok, Error)
  }
}
...
module Bifoldable: BIFOLDABLE with type t<'a, 'b> = result<'a, 'b> = {
  type t<'a, 'b> = result<'a, 'b>
