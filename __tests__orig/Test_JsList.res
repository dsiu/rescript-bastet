...
module ArbitraryList: Test.ARBITRARY_A
  with type t<'a> = list<'a>
...
module ArbitraryListInt: Test.ARBITRARY
  with type t = list<int>
  and type arbitrary<'a> = arbitrary<'a> = {
  type t = list<int>
...
describe("List'", () => MochaI.run(TestList.suites))
